...
Calculator::Calculator() {
	...
	_calc_features = std::make_unique<CalcFeatures>(available_functions);
}

Calculator::~Calculator() {
	_calc_features = nullptr;
...
std::vector<std::string> Calculator::ConvertToStandartForm(const std::vector<std::string>& tokens) {
	...
	while (i < tokens.size()) {
		if (tokens.at(i) == "(") {
			...
			if (_calc_features->isNumber(tokens.at(i + 1)) ||
				_calc_features->isFunction(tokens.at(i + 1)) ||
				tokens.at(i + 1) == "(")
			{
...
std::stringstream Calculator::ConvertToRPN(std::vector<std::string> tokens) {
	...
	for (std::string token : tokens) {
		if (_calc_features->isNumber(token)) {
...
void Calculator::Solve(std::string input_expression) {
	...
	std::string answer = _calc_features->Calculate(rpn_expression);
